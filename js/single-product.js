...
const producutID = localStorage.getItem("productId")
  ? JSON.parse(localStorage.getItem("productId"))
  : localStorage.setItem("productId", JSON.stringify(1));


const producuts = localStorage.getItem("products")
  ? JSON.parse(localStorage.getItem("products"))
  : localStorage.setItem("products", JSON.stringify([]));

 

const findProduct = producuts.find((item) => item.id === Number(producutID));
...
const productsThumbs = document.querySelectorAll(".product-thumb .glide__slide img")

productsThumbs[0].classList.add("active")
